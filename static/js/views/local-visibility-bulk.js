/* #/local-visibility/bulk-scoring — upload a lead-list CSV/Excel file,
   create an Advice Local "client" per row (see advicelocal_client.py) and
   poll for its auto-generated baseline visibility score, then hand back
   the same file with two columns appended (Local Visibility Score, Local
   Visibility Grade). Businesses created here use lead_source=
   "local_visibility_csv" and are deliberately excluded from the main
   dashboard (see routes.py's list_businesses). Split out of the old
   single #/local-visibility view into its own page, now a subcategory of
   the Local Visibility Dashboard (#/local-visibility) alongside GHL
   Leads and Manual Scoring. Mirrors static/js/views/bulk-scoring.js's
   structure closely for the website-audit side. */
window.WVA = window.WVA || {};
WVA.views = WVA.views || {};

WVA.views.localVisibilityBulk = async function (main) {
    const U = WVA.ui;
    ...
    function pageHtml() {
        return U.pageHead({
            title: 'Local Visibility Bulk Scoring',
            sub: 'Upload a lead list, score each business\'s local search presence via Advice Local, and download the same file back with two new columns added.',
            back: { href: '#/local-visibility', label: 'Local Visibility Dashboard' },
        }) + `<div class="card card-pad form-card" id="lv-upload-card">
            <div class="logo-upload-title">Upload lead list (CSV or Excel)</div>
            <div class="text-sm muted mt-8">
                Needs a Business Name column and a Zip/Postal Code column. Optional columns
                (Street Address, City, State, Phone, Email, Website, First Name, Last Name)
                improve accuracy and are detected automatically — every other column in your
                file is preserved as-is.
            </div>
            <div class="mt-16">
                <input type="file" id="lv-file" accept=".csv,.xlsx" hidden>
                <button type="button" class="btn btn-primary" id="lv-upload-btn">
            ...
            <div id="lv-active-progress"></div>
        </div>
        <div id="lv-jobs-holder" class="mt-16">${U.loading('Loading upload history…')}</div>`;
    }

    function wireUpload() {
        const fileInput = main.querySelector('#lv-file');
        const btn = main.querySelector('#lv-upload-btn');
        ...
        fileInput.addEventListener('change', async () => {
            ...
            try {
                ...
                const res = await fetch('/api/local-visibility/upload', { method: 'POST', body: fd });
                let data = null;
                ...
                U.toast(`Scoring ${data.total_rows} row${data.total_rows === 1 ? '' : 's'} in the background — this can take a while per row (external API), feel free to navigate away and come back.`, 'success');
                resumedPoll = true;
                pollJob(data.job_id);
                await refreshJobs();
            } catch (err) {
    ...
    function pollJob(jobId) {
        const tick = async () => {
            ...
            let job;
            try {
                job = await WVA.api.get(`/api/local-visibility/jobs/${jobId}`);
            } catch (e) {
                return;
            }
            const holder = main.querySelector('#lv-active-progress');
            if (job.status === 'running') {
                ...
                if (job.status === 'completed') {
                    U.toast('Local visibility scoring finished — download is ready.', 'success');
                } else if (job.status === 'failed') {
                    U.toast(job.error_message || 'Local visibility scoring job failed.', 'error');
                }
            }
        };
        tick();
    ...
    async function refreshJobs() {
        const holder = main.querySelector('#lv-jobs-holder');
        if (!holder) return;
        let jobs;
        try {
            jobs = await WVA.api.get('/api/local-visibility/jobs');
        } catch (e) {
        ...
        const rows = jobs.map((j) => {
            ...
            const download = j.download_available
                ? `<a class="btn btn-secondary btn-sm" href="/api/local-visibility/jobs/${U.esc(j.id)}/download" download>Download</a>`
                : '<span class="muted">—</span>';
            const view = j.download_available
                ? `<button type="button" class="btn btn-secondary btn-sm lv-view-btn" data-job-id="${U.esc(j.id)}">View</button>`
                : '';
            return `<tr class="lv-job-row" data-job-id="${U.esc(j.id)}">
            ...
            <tr class="lv-results-row" data-job-id="${U.esc(j.id)}" style="display:none;">
                <td colspan="5"></td>
            </tr>`;
        }).join('');
        ...
        holder.querySelectorAll('.lv-view-btn').forEach((btn) => {
    ...
    async function toggleResults(jobId, btn) {
        const holder = main.querySelector('#lv-jobs-holder');
        const resultsRow = holder.querySelector(`.lv-results-row[data-job-id="${cssEscape(jobId)}"]`);
        ...
        if (!cell.dataset.loaded) {
            ...
            try {
                const data = await WVA.api.get(`/api/local-visibility/jobs/${jobId}/results`);
