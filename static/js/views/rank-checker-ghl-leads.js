...
WVA.views.rankCheckerGhlLeads = async function (main) {
    ...
    let scans;
    try {
        scans = await WVA.api.get('/api/rank-checker/scans?lead_source=rank_checker_ghl');
    } catch (e) {
    ...
    main.innerHTML = pageHead() + `<div class="table-wrap">
        <table class="table">
            <thead><tr><th>Business</th><th>Contact</th><th>Website</th><th>Keywords</th><th>Found</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead>
            <tbody>${rows}</tbody>
        </table>
    </div>`;
