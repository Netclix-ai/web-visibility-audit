...
# Playwright (headless Chromium) — install to a project-local, writable path
export PLAYWRIGHT_BROWSERS_PATH="$(pwd)/.playwright-browsers"
if [ ! -d "$PLAYWRIGHT_BROWSERS_PATH" ] || [ -z "$(find "$PLAYWRIGHT_BROWSERS_PATH" -maxdepth 1 -name 'chromium-*' 2>/dev/null)" ]; then
  echo "[+$(elapsed)ms] Installing Playwright chromium..."
  uv run playwright install chromium
  echo "[+$(elapsed)ms] Playwright chromium installed"
else
  echo "[+$(elapsed)ms] Playwright chromium already installed"
fi
